...
export const getUserProfileFromToken = () => (dispatch, getState) => {
  ...
  request({ url: '/users/profile', method: 'GET', dispatch, getState })
    .then(data => {
      dispatch(setProfileData(data));
    })
    .catch(error => {});
...
export const refreshTokenRequest = () => (dispatch, getState) =>
  new Promise((resolve, reject) => {
    request({ url: '/auth/refresh-token', method: 'POST', isRefresh: true, dispatch, getState })
      .then(data => {
        dispatch(setTokenData(data));
        resolve(resolve);
      })
...
export const saveProfile = ({
  ...
  new Promise(resolve => {
    ...
    request({ url: '/users/profile', method: 'PATCH', data, getState, dispatch })
      .then(data => {
        dispatch(setProfileData(data));
